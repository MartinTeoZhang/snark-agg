...
pub fn verify_aggregate_proof<E: Pairing + std::fmt::Debug, R: Rng + Send, T: Transcript + Send>(
    ...
    proof.parsing_check()?;
    //gamma_abc_g1是一个椭圆曲线点的数组，它包含了用于验证证明的关键参数。这些参数与证明中的公共输入一一对应，并且额外包括一个常数项（通常是第一个元素）。因此，公共输入的数量加上一个常数项应该等于gamma_abc_g1数组的长度。
    for pub_input in public_inputs {
        if (pub_input.len() + 1) != pvk.vk.gamma_abc_g1.len() {
            return Err(Error::MalformedVerifyingKey);
        }
    }
    println!("proof.tmipp.gipa.nproofs.len() = {}", proof.tmipp.gipa.nproofs);
